...
@section('content')
    <div class="flex h-full w-full mb-10">
        ...
        <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="w-full px-5">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-2 gap-4 mb-4">
                <!-- Form Fields -->
                <div>
                    <label for="id" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Mã sản phẩm</label>
                    <input type="text" name="id" id="id" value="{{ $product->id }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="" required readonly>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="brand" id="brand" value="{{ $product->brand }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="" required>
                </div>
                <div class="mb-4">
                    ...
                    <div class="relative mt-2 rounded-md shadow-sm">
                        ...
                        <input type="hidden" name="price_numeric" id="price_numeric" value="{{ old('price_numeric', $product->price) }}">
                        <input type="text" name="price" id="price" value="{{ number_format($product->price) }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 pl-7 pr-12 text-right dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="0" aria-describedby="price-currency" required>
                ...
                <div class="col-span-2">
                    ...
                    <div class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white overflow-auto" style="max-height: 125px;">
                        <div class="grid grid-cols-2 gap-4 mb-2">
                            @foreach ($categories as $category)
                                <div class="flex items-center mb-2">
                                    <input type="checkbox" id="category-{{ $category->id }}" name="category[]" value="{{ $category->id }}" {{ $product->categories->contains($category->id) ? 'checked' : '' }} class="mr-2">
                ...
                <div class="col-span-2">
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-2">
                        @foreach ($warehouses as $warehouse)
                            <div class="mb-4">
                                <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ $warehouse->name }}</label>
                                <?php
                                $inventory = $warehouses_inventory->where('warehouse_id', $warehouse->id)->first();
                                $quantity = $inventory ? $inventory->quantity : 0;
                                ?>
                                <input type="number" name="warehouse_quantity[{{ $warehouse->id }}]" id="warehouse_quantity_{{ $warehouse->id }}" value="{{ $quantity }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Số lượng">
                            </div>
                        @endforeach
                    </div>
                </div>
                <div class="mb-4">
                    ...
                    <input type="file" name="image_path" id="image_path" class="mt-1 block w-full text-sm text-gray-500 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500">
                    <div id="image-preview" class="mt-2">
                        <p id="new-image-message" style="display: none;" class="mt-10 block text-sm font-medium text-gray-900 dark:text-white">Ảnh mới</p>
                    ...
                    @if ($product->image_path)
                        <div class="mt-2">
                            ...
                            <img src="{{ asset($product->image_path) }}" alt="Product Image" class="mt-2" style="max-width: 200px;">
                        </div>
                    @endif
                    @error('image_path')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    ...
                    reader.onload = function (event) {
                        ...
                        img.onload = function () {
                            ...
                            if (width === height) {
                                ...
                                previewImage.style.display = 'none'; // Hide the preview if not 1:1 ratio
